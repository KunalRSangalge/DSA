...
int main(){
    ...
    while(t--){
        ...
        input(b,n);
        for (int i = 0; i < n; ++i) {
            initial_v += abs(a[i] - b[i]);
        }
        if(k==0) {
            cout<<initial_v<<endl; continue;
        }
        ...
        bool flag = false;
        ll maxi = intervals[0].second;
        for (int i = 1; i < n; ++i) {
            if (intervals[i].first <= maxi) {
                flag = true;
                break;
            }
        ...
        if (flag) {
            ...
            ll mini = -1; 
            for (int i=0;i<n-1;++i) {
                ll cur=intervals[i+1].first - intervals[i].second;
                if (mini==-1 || cur<mini) {
                    mini = cur;
                }
            }
            cout << initial_v + 2*mini << endl;
        }
    }
}
