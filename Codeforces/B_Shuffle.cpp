...
int main(){
    ...
    while(t--){
        ...
        // for(int i=0;i<m;i++)cout<<v[i].first<<" "<<v[i].second;cout<<endl;
        ll mini=0,maxi=0;
        bool flag=false;
        for(int i=0;i<m;i++){
            if(!flag){
                if(x>=v[i].first && x<=v[i].second){maxi=v[i].second;mini=v[i].first; flag=true;}
            }
            else{
                if(maxi<v[i].first || mini>v[i].second){continue;}
                else {
                    mini=min(v[i].first,mini);
                    maxi=max(v[i].second,maxi);
                }
            }
        }
        if(flag) cout<<maxi-mini+1<<endl;
        else cout<<1<<endl;
    }
}
