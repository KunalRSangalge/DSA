...
int findMEXInRange(vector<ll>& arr, int i, int j) {
...
int main(){
    ...
    while(t--){
        ...
        vector<ll>mexes;
        if(zidx[0]!=0){
            mexes.push_back(findMEXInRange(a,0,zidx[0]+1));
        }
        for(int i=0;i<zidx.size()-1;i++){
            int x = findMEXInRange(a,zidx[i],zidx[i+1]);
            mexes.push_back(x);
        }
        
        for(int i=0;i<mexes.size();i++)cout<<mexes[i]<<" "; cout<<endl;
    }
}
