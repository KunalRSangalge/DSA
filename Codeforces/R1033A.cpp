...
int main(){
    ...
    while(t--){
        ...
        bool flag=false;
        if(l1==l2 && l2==l3){
            if(l1 == (b1+b2+b3))flag=true;
        }
        if(b1==b2 && b2==b3){
            if(b1 == (l1+l2+l3))flag=true;
        }
        vector<ll>l={l1,l2,l3};
        vector<ll>b={b1,b2,b3};
        sort(all(l));
        sort(all(b));
        if(b[0]==b[1] && l[0]+l[1]==l[2] && l[2]==b[0]+b[2])flag=true;
        if(l[0]==l[1] && b[0]+b[1]==b[2] && b[2]==l[0]+l[2])flag=true;
        if(flag)yes();
        else no();
    }
}
