...
int main(){
    ...
    while(t--){
        ...
        cin>>n>>l>>r;
        ll a[n];
        // ll left[n]={INT_MAX},right[n]={INT_MAX},mid[r-l+1]={INT_MAX};
        vector<ll> left(n,INT_MAX);
        vector<ll> right(n,INT_MAX);
        vector<ll> mid(n,INT_MAX);
        for(int i=0;i<n;i++){cin>>a[i];}
        if(l!=1){
            for(int i=0;i<l-1;i++)left[i]=a[i];
