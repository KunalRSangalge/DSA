...
void build(ll ind,ll low,ll high,ll a[],ll seg[],ll orr){
    if(low==high){
        seg[ind]=a[low];
        return;
    }
...
void update(ll ind,ll low,ll high,ll seg[],ll i,ll val,ll orr){
    if(low==high){
        seg[ind]=val;
        return;
    }
    ll mid = (low+high)/2;
    if(i<=mid){
        update(2*ind+1,low,mid,seg,i,val,!orr);
    }
...
int main(){
    ...
    while(t--){
        ...
        ll seg[4*el+1];
        ll orr=false;
        if(n%2==0) build(0,0,el-1,a,seg,0);
        else build(0,0,el-1,a,seg,1);
