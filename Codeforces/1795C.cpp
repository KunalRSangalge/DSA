...
ll f(ll low,ll high,ll num,ll k,vector<ll>&pref){
    ll ans=-1;
    while(low<=high){
        ll mid = (low+high)/2;
        if(pref[mid]-num < k){low=mid+1;ans=mid-1;}
        else high=mid-1;
    }
    return ans;
}
int main(){
    ...
    while(t--){
        ...
        vector<ll>full(n+1,0),extra(n+1,0);
        vector<ll>ans(n);
        //difference array -> updating in O(1)
        vector<pair<ll,ll>>query(n);
        for(int i=0;i<n;i++){
            ll l=i;
            ll r=f(i+1,n,pref[i],a[i],pref);
            query[i]={l,r};
            if(r>=0) extra[r+1]+=(a[i]-(pref[r+1]-pref[l]));
            else extra[l]+=a[i];
        }
