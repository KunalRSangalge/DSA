...
ll f(int i,int sum,vector<ll>&v,vector<vector<ll>>&dp){
    if(sum==0)return 0;
    if(i==0){
    ...
    ll pick=INT_MAX,notpick=0;
    if(sum>=v[i])pick = 1+f(i,sum-v[i],v,dp);
    notpick = f(i-1,sum,v,dp);
    // for(int j=0;j<v.size();j++){
    //     ll pick=0,notpick=0;
    //     if(sum>=v[j])pick = 1+f(j,sum-v[j],v,dp);
    //     notpick = f(j-1,sum,v,dp);
...
int main(){
    ...
    while(t--){
        ...
        for(int i=1;i<=x;i++){
            for(int j=0;j<n;j++){
               if(i>=v[j]) dp[i]=min(dp[i],1+dp[i-v[j]]);
            }
        }
