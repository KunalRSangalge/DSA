...
ll f(string& s,ll i,ll j,string& check,vector<vector<ll>>&dp,ll n,ll m){
    if(j==m)return 1;
    if(i==n)return 0;
    if(dp[i][j]!=-1)return dp[i][j];
    ll pick=0,notpick=0;
    if(check[j]==s[i]){
        pick=f(s,i+1,j+1,check,dp,n,m)%mod;
        notpick=f(s,i+1,j,check,dp,n,m)%mod;
        return dp[i][j] = (pick+notpick)%mod;
    }
    else return dp[i][j] = f(s,i+1,j,check,dp,n,m)%mod;
...
int main(){
    ...
    while(t--){
        ...
        // ll ans = f(s,0,0,check,dp,n,m)%mod;
        // cout<<ans%mod<<endl;
